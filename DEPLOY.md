...
| 名字 | 是什么 | 哪来的 | 用在哪 |
|---|---|---|---|
| `WECOM_WEBHOOK` | 企业微信机器人地址，**真正的凭据** | 微信群 → 群机器人 → 复制 | 发消息用，三种方式都要 |
| `PUSH_TOKEN` | 你自己随口定的一串访问口令，保护**本项目的网站接口** | 自己编，不是任何平台发的 | 仅方式 1、2；方式 3 用不到 |

不设 `PUSH_TOKEN` 会怎样：`server.js` 默认只监听 `127.0.0.1`，本机访问无所谓。但一旦 `--host=0.0.0.0` 挂公网，`POST /api/push` 就成了公开接口，任何人扫到端口就能借你的机器人往群里发任意内容。设了口令之后，网页打开会弹框要求输入一次，之后存在浏览器里。
